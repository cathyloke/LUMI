...
const CustomStackContent = ({ navigation }: any) => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 20 }}>
        <TouchableOpacity onPress={() => setQuantity(q => Math.max(1, q - 1))} style={{ marginRight: 20 }}>
          <Ionicons name="remove-circle-outline" size={32} color="#102C57" />
        </TouchableOpacity>
        <Text style={{ fontSize: 24, fontWeight: 'bold', color: '#102C57' }}>{quantity}</Text>
        <TouchableOpacity onPress={() => setQuantity(q => q + 1)} style={{ marginLeft: 20 }}>
          <Ionicons name="add-circle-outline" size={32} color="#102C57" />
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
